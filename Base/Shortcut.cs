...
namespace QuickShortCut.Base
{
    class Shortcut
    {
        ...
        public static bool CreateShortcut(string srcFilePath, string dstFolderPath, string args = "")
        {
            bool flag = true;
            ShellLink shortcut;
            try
            {
                if (File.Exists(srcFilePath))
                {
                    ...
                    {
                        WorkPath = Path.GetDirectoryName(srcFilePath),
                        IconLocation = srcFilePath + ",0",
                        ExecuteFile = srcFilePath,
                        ExecuteArguments = args
                    };
                    if (!File.Exists(dstFolderPath + @"\" + fileName + ".lnk"))
                    {
                        shortcut.Save(dstFolderPath + @"\" + fileName + ".lnk");
                    }
                    else
                    {
                        DirectoryInfo di = new DirectoryInfo(srcFilePath);
                        shortcut.Save(dstFolderPath + @"\" + fileName + "_" + di.Name + ".lnk");
                    }
                    shortcut.Dispose();
                }
                else if (Directory.Exists(srcFilePath))
                {
                    shortcut = new ShellLink
                    {
                        //WorkPath = Path.GetDirectoryName(srcFilePath),
        ...
        /// <summary>
        /// .NET Framework 3.5 或更早期版本
        /// </summary>
        /// <param name="linkFileName"></param>
        /// <param name="args"></param>
        private static void CreateShortcutOld(string linkFileName, string args = "")
        {
